...
const statusMeta: Record<string, { icon: typeof Clock; className: string; label: string }> = {
  ...
  processing: { icon: Loader2, className: 'text-warning', label: 'Processing' },
  posted: { icon: CheckCircle2, className: 'text-success', label: 'Posted' },
};
...
function ScheduledRow({ post }: { post: ScheduledPost }) {
  ...
  return (
    <div className="bg-surface border border-line rounded-xl p-3 flex items-center gap-4 shadow-main hover:border-line-2 transition-colors fade-up">
...
function Loading() {
  return (
    <div className="flex items-center justify-center py-16 text-ink-5 text-[13px] gap-2">
      <Loader2 size={14} className="animate-spin text-accent" /> Loading from post-bridge...
    </div>
  );
}
